...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // Alternative fade: 1.0, 1.0 - easedProgress, or 1.0 - exponentialDecayPulse(rippleProgress).

    // Work in pixels. This is algebraically equivalent to the former normalized coordinates.
    float rippleRadius = easedProgress * MAX_RADIUS * iResolution.y * 0.5;
    float halfThickness = RING_THICKNESS * iResolution.y * 0.25;
    float blurWidth = max(BLUR, 0.0);
    vec2 cursorCenter = iCurrentCursor.xy + iCurrentCursor.zw * vec2(0.5, -0.5);
    vec2 fromCursor = fragCoord - cursorCenter;
    float distanceSquared = dot(fromCursor, fromCursor);
    float outerRadius = rippleRadius + halfThickness + blurWidth;

    if (distanceSquared > outerRadius * outerRadius) {
        return;
    }

    float innerRadius = rippleRadius - halfThickness - blurWidth;
    if (innerRadius > 0.0 && distanceSquared < innerRadius * innerRadius) {
        return;
    }

    float sdfRing = abs(sqrt(distanceSquared) - rippleRadius) - halfThickness;
    float ripple = getEffectCoverage(sdfRing, blurWidth) * fade;
