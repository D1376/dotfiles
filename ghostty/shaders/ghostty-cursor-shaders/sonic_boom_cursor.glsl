...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float outerRadius = rippleRadius + blurWidth;

    // The smoothstep contribution is exactly zero outside this radius.
    if (distanceSquared > outerRadius * outerRadius) {
        return;
    }

    float sdfCircle = sqrt(distanceSquared) - rippleRadius;
    float ripple = getEffectCoverage(sdfCircle, blurWidth) * fade;
