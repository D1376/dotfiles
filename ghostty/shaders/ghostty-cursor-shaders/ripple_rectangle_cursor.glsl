...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float edgeDistance = max(distanceToEdge.x, distanceToEdge.y);
    float band = halfThickness + blurWidth;

    // Cull both the outer area and the deep interior where the ring contributes zero.
    if (edgeDistance > band || edgeDistance < -band) {
        return;
    }

    float sdfRectangle = length(max(distanceToEdge, 0.0)) + min(edgeDistance, 0.0);
    float sdfRing = abs(sdfRectangle) - halfThickness;
